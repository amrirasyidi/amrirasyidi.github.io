...
date: 2023-09-22
excerpt: Using optuna for XGBoost experimentation in time series forecasting use case
tags: [data-science, how-to, python]
image: "/assets/blog/images/20230922_6_feature_importance.png"
---
...
<Figure src="/assets/blog/images/20230922_1_header.png" alt="header">
  What a title huh?
</Figure>
...
2. **Metrics yielded** — variables of our choosing to decide which model performs the best
3. **Model** — this one is kind of obvious
4. **Artifacts** — as far as I understand, this can be any file or directory. So far I've stored:
   1. image
   2. record of metrics as a `json` file
...
1. [Data preparation](#data-preparation)
   ...
   2. Time series data splitting — training, validation, and test
   3. Turn it into time series forecasting form
2. [Model training pipeline](#model-training-pipeline)
   1. Prepare the hyperparameters
   2. Calculate the metrics
   3. Visualize the result
3. [Inferencing pipeline](#inferencing-pipeline)
...
<Figure src="/assets/blog/images/20230922_2_train_data.png" alt="head of training data">
  train_x.head(2)
</Figure>
...
<Figure src="/assets/blog/images/20230922_3_training_pred.png" alt="prediction of training data">
...
<Figure src="/assets/blog/images/20230922_4_test_pred.png" alt="prediction of test data">
...
That's it for now, the additional logging will come later in the [enhanced experiment](#enhanced-experiment) part.

## Hyperparameter Optimization

Now come the Optuna part. As mentioned earlier, we need to make a trial and a study.

First, we have to turn our training and prediction pipeline into a function called `objective`, which I *think* is the trial object.
...
2. Then we call the `study.optimize()` using our custom function `objective`, and define how many trials we are going to try. In this case, since we are just trying out the syntax, we choose 2 so we don't waste so much time. Just to see is it working or not.
...
<TwoCol>
  <Figure slot="left" src="/assets/blog/images/20230922_5_optim_history.png" alt="optimization history">
    optimization history plot
  </Figure>
  <Figure slot="right" src="/assets/blog/images/20230922_6_feature_importance.png" alt="feature importance">
...
<Callout type="idea">
  in case you want to see the `evals_result`, you are required to define the `evals` variables
</Callout>
...
3. We use `early_stopping_rounds` in case a trial is not better after several epoch, so it will stop early before reaching the complete epoch (`num_round`), hence saving our resources.
